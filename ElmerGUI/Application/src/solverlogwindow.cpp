...
SolverLogHighlighter::SolverLogHighlighter(int type, QTextDocument *parent)
  ...
  if (type != SOLVERLOG_HIGHLIGHTING_LIGHT &&
      type != SOLVERLOG_HIGHLIGHTING_DARK)
    return;
...
SolverLogWindow::SolverLogWindow(QWidget *parent) : QMainWindow(parent) {
  ...
  QString strFont =
      ((MainWindow *)parent)
          ->settings_value(QString("solverlogWindow/font"), QString(""))
          .toString();
  QFont font;
  ...
  int syntaxHighlighting =
      ((MainWindow *)parent)
          ->settings_value(QString("solverlogWindow/highlighting"),
                           SOLVERLOG_HIGHLIGHTING_NONE)
          .toInt();
...
QTextEdit *SolverLogWindow::getTextEdit(void) { return this->textEdit; }

void SolverLogWindow::setFirstTime(bool b) { this->firstTime = b; }

void SolverLogWindow::setFound(bool b) { this->found = b; }

QSize SolverLogWindow::minimumSizeHint() const { return QSize(64, 64); }
...
void SolverLogWindow::fontSlot() {
  ...
  ((MainWindow *)parent())
      ->settings_setValue(QString("solverlogWindow/font"), font.toString());
}
void SolverLogWindow::highlightingNoneSlot() {
  ...
  highlighter = NULL;
  ((MainWindow *)parent())
      ->settings_setValue(QString("solverlogWindow/highlighting"),
                          SOLVERLOG_HIGHLIGHTING_NONE);
...
void SolverLogWindow::highlightingLightSlot() {
  ...
  setStyleSheet(style);
  delete highlighter;
  highlighter = new SolverLogHighlighter(SOLVERLOG_HIGHLIGHTING_LIGHT,
                                         textEdit->document());
  ((MainWindow *)parent())
      ->settings_setValue(QString("solverlogWindow/highlighting"),
                          SOLVERLOG_HIGHLIGHTING_LIGHT);
...
void SolverLogWindow::highlightingDarkSlot() {
  ...
  setStyleSheet(style);
  delete highlighter;
  highlighter = new SolverLogHighlighter(SOLVERLOG_HIGHLIGHTING_DARK,
                                         textEdit->document());
  ((MainWindow *)parent())
      ->settings_setValue(QString("solverlogWindow/highlighting"),
                          SOLVERLOG_HIGHLIGHTING_DARK);
  highlightingNoneAct->setChecked(false);
  highlightingLightAct->setChecked(false);
